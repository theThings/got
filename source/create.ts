...
const create = (defaults: InstanceDefaults): Got => {
	...
	// Pagination
	const paginateEach = (async function <T, R>(url: string | URL, options?: OptionsWithPagination<T, R>): Promise<AsyncIterableIterator<T>> {
	...
	got.paginate = paginateEach as unknown as GotPaginate;

	got.paginate.all = (async <T, R>(url: string | URL, options?: OptionsWithPagination<T, R>) => {
		const results: T[] = [];

		for await (const item of await paginateEach<T, R>(url, options)) {
			results.push(item);
		}
	...
	got.paginate.each = paginateEach as unknown as GotPaginate['each'];
...
export * from './types';
function setTimeout(resolve: (value: unknown) => void, ms: number) {
